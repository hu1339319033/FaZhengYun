...
//        sharedPreferUtil=SharedPref.getInstance(context);
...
//    SharedPref sharedPreferUtil=null;
    ...
    public void startSound(final MusicType musicType){
        boolean musicFlag=sharedPreferences.getBoolean(Constants.MUSIC_FLAG, true);
//      if(musicFlag){
...
//      }
    ...
    @SuppressLint("UseSparseArrays") private void initSoundPool(){
        ...
        try {
            sources.put(0, sp.load(context.getAssets().openFd("msg.wav"),1));
            sources.put(1, sp.load(context.getAssets().openFd("msg.wav"),1));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
